...
penguins%>%
  ...
  summarise(mean_bill_lenght= mean(bill_length_mm, na.rm=TRUE), mean_billdepth= mean(bill_depth_mm, na.rm=TRUE), mean_flipperlength= mean(flipper_length_mm, na.rm=TRUE), mean_bodymass= mean(body_mass_g, na.rm=TRUE), total=n())
```

```
## # A tibble: 1 x 5
##   mean_bill_lenght mean_billdepth mean_flipperlength mean_bodymass total
##              <dbl>          <dbl>              <dbl>         <dbl> <int>
...
penguins%>%
  group_by(species)%>%
  summarise(mean_billlength= mean(bill_length_mm, na.rm=TRUE))
```
